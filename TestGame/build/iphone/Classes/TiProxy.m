...
//Rare exceptions to indicate a bug in the _testgame code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_testgame = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_testgame]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.testgame.module"
